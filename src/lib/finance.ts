...
export function calcularItem(
  ...
  desconto = 0,
  percentualRepasse = PERCENTUAL_REPASSE_PADRAO,
): ResultadoFinanceiro {
...
export function somarResultados(itens: ResultadoFinanceiro[]): ResultadoFinanceiro {
  return itens.reduce<ResultadoFinanceiro>(
    (acc, r) => ({
...
export function metricasProduto(p: {
  ...
  const margem =
    p.preco_venda > 0 ? round2((lucroUnidade / p.preco_venda) * 100) : 0
